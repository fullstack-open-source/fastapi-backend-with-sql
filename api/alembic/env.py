"""
Alembic Migration Environment
Imports all models for autogenerate support
"""
import os
import sys
from logging.config import fileConfig
from pathlib import Path
from urllib.parse import quote_plus

from sqlalchemy import engine_from_config
from sqlalchemy import pool

from alembic import context

# Add the api directory to Python path so we can import models
sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

# Load environment variables first
from dotenv import load_dotenv
load_dotenv()

# this is the Alembic Config object, which provides
# access to the values within the .ini file in use.
config = context.config

# Interpret the config file for Python logging.
# This line sets up loggers basically.
if config.config_file_name is not None:
    fileConfig(config.config_file_name)

# Import Base and all models for autogenerate support
# All models must be imported so they're registered with Base.metadata
# Import Base first (it may import logger, but we've fixed the logger issue)
try:
    from src.db.models.base import Base
except Exception as e:
    # If import fails, provide helpful error message
    import sys
    print(f"Error importing models: {e}", file=sys.stderr)
    print("Make sure all dependencies are installed and environment variables are set correctly.", file=sys.stderr)
    raise

# Set target_metadata for autogenerate
target_metadata = Base.metadata

# Build database URL from environment variables (always override alembic.ini)
# This ensures we use the correct database connection from environment
DATABASE_HOST = os.environ.get('DATABASE_HOST')
DATABASE_NAME = os.environ.get('DATABASE_NAME')
DATABASE_USER = os.environ.get('DATABASE_USER')
DATABASE_PASSWORD = os.environ.get('DATABASE_PASSWORD')
DATABASE_PORT = os.environ.get('DATABASE_PORT', '5432')

# Check if DATABASE_URL is provided directly
DATABASE_URL = os.environ.get('DATABASE_URL')

if not DATABASE_URL:
    # Build from individual components
    if all([DATABASE_HOST, DATABASE_NAME, DATABASE_USER, DATABASE_PASSWORD]):
        # URL encode password to handle special characters
        from urllib.parse import quote_plus
        encoded_password = quote_plus(DATABASE_PASSWORD)
        DATABASE_URL = f"postgresql://{DATABASE_USER}:{encoded_password}@{DATABASE_HOST}:{DATABASE_PORT}/{DATABASE_NAME}"
    else:
        missing = []
        if not DATABASE_HOST: missing.append('DATABASE_HOST')
        if not DATABASE_NAME: missing.append('DATABASE_NAME')
        if not DATABASE_USER: missing.append('DATABASE_USER')
        if not DATABASE_PASSWORD: missing.append('DATABASE_PASSWORD')
        raise ValueError(f"Missing required database environment variables: {', '.join(missing)}")

# Always set the database URL (override alembic.ini)
config.set_main_option("sqlalchemy.url", DATABASE_URL)
print(f"🔗 Connected to database: {DATABASE_USER}@{DATABASE_HOST}:{DATABASE_PORT}/{DATABASE_NAME}")

# other values from the config, defined by the needs of env.py,
# can be acquired:
# my_important_option = config.get_main_option("my_important_option")
# ... etc.


def run_migrations_offline() -> None:
    """Run migrations in 'offline' mode.

    This configures the context with just a URL
    and not an Engine, though an Engine is acceptable
    here as well.  By skipping the Engine creation
    we don't even need a DBAPI to be available.

    Calls to context.execute() here emit the given string to the
    script output.

    """
    url = config.get_main_option("sqlalchemy.url")
    context.configure(
        url=url,
        target_metadata=target_metadata,
        literal_binds=True,
        dialect_opts={"paramstyle": "named"},
    )

    with context.begin_transaction():
        context.run_migrations()


def run_migrations_online() -> None:
    """Run migrations in 'online' mode.

    In this scenario we need to create an Engine
    and associate a connection with the context.

    """
    connectable = engine_from_config(
        config.get_section(config.config_ini_section, {}),
        prefix="sqlalchemy.",
        poolclass=pool.NullPool,
    )

    with connectable.connect() as connection:
        context.configure(
            connection=connection,
            target_metadata=target_metadata,
            compare_type=True,  # Detect column type changes
            compare_server_default=True,  # Detect default value changes
        )

        with context.begin_transaction():
            context.run_migrations()


if context.is_offline_mode():
    run_migrations_offline()
else:
    run_migrations_online()
